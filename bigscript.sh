    ...
    search_mp3() {
        ...
        # Wyświetlanie informacji o plikach z tablicy posortowanej
        echo ""
        for info in "${sorted_files_info[@]}"; do
            echo "$info"
        done
    }
    ...
    case $option in
        # Option to browse mp3 files and their tags
        1)
        ...
        3)
            ...
            read -p "Enter file paths separated by space to add to the playlist: " -a selected_files
            create_playlist "$playlist_name" "${selected_files[@]}"
            ;;
        *)
            echo "Unknown option."
            ;;
